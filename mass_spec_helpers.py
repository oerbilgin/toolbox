...
def find_mass(target_weight, ppm, df, mz_column_name='mz', rt_column_name='rt_peak', rt=None, rt_bound=0.05):
    """
    given a target mass and ppm window, return the rows of a dataframe corresponding to the mass. 
    ...
    mz_column_name: the column name that has the feature's m/z, metatlas default: "mz"
    rt_column_name: the column name that has the reature's retention time peak, metatlas default: "rt_peak
    rt: the retention time for the feature
    rt_bound: the upper and lower bound you're willing to search for the retention time, default +/- 0.05 minutes
...
def data_convert(row, data_pattern='Peak height', search='endswith'):
    """
    Converts an input Series into a dataframe with 'x' and 'y' columns, with 'x' corresponding to run order, and 'y' corresponding to a measurement of interest
    Run number is determined by splitting by the word "Run" and then by a period. For example: "experimentinfo_Run1.mzml"
    If your filenames don't look like that, then the run number finder probably won't work
    
    Inputs
    ------
    row: a Pandas Series object, where the colums contain run information
    data_pattern: is the text pattern of the column that contains data you want to plot, default: "Peak height"
    search: the Pandas string methods search type:
        if search is "endswith" or "startswith" then it will be a str.endswith or str.startswith search; multiple data_patterns to search should be in a TUPLE
        if search is "contains" then it will be a str.contains search; multiple data_patterns to search should be in one string separated by '|'
        
    Outputs
    -------
    A dataframe with columns 'x' and 'y' that is sorted in ascending order of 'x'
    """
    ...
    if search.lower()=='endswith':
        ...
        raise RuntimeError('Did not recognize %s as a valid Pandas search pattern, or it is not encompassed yet' %(search))
...
def ctrl_expt_compare(medium, experimental, fold_change=4):
    ...
    Input dataframes are data_convert() outputs! (i.e. 'x' and 'y' columns corresponding to run number and ion intensity, respectively)
    Compares an experimental data point with the medium controls that were run within a 20-run window (+/- 10 runs) of the experimental
    ...
    Inputs
    ------
    medium: data_convert() output corresponding to medium control runs for one feature
    experimental: data_convert() output corresponding to experimental condition runs for one feature
    fold_change: boundary to toss a feature if found to be fold_change higher in experimental over medium, or 1/fold_change lower in experimental over medium; default 4
    
    Outputs
    -------
    boolean True or False, where True means that the feature is has a very different intensity (as judged by fold_change) in experimental compared to the nearest control run
    """
    # for each medium control point, look for experimental points within 10 runs of a control point
    for idx, ctrl in medium.iterrows():
        expt = experimental[(experimental['x'] < (ctrl['x']+10)) & (experimental['x'] > (ctrl['x']-10))]   
            ...
            for y in expt['y']:
                ...
                # otherwise, look at the fold difference between experimental and control
                    ...
                    if fold > fold_change or fold < (1 / fold_change):
                        continue
                    # if the experimental is ever within a 0.25 - 4x range of the control, toss it immediately
                    else:
                        return False
